...
function parseViewCount(viewsStr: string): number {
  ...
  const cleaned = viewsStr.replace(/[KM]/gi, '');
  const multiplier = viewsStr.toUpperCase().includes('K') ? 1000 : 
                    viewsStr.toUpperCase().includes('M') ? 1000000 : 1;
...
function parseMessagesFromHTML(
  ...
  $('.tgme_widget_message').each((_, messageEl) => {
    ...
    $message.find('.tgme_reaction').each((_, reactionEl) => {
      ...
      const emoji = $reaction.find('i b').text().trim();
      const countText = $reaction.text().replace(emoji, '').trim();
      const count = parseInt(countText, 10) || 0;
      if (emoji && count > 0) reactions.push({ emoji, count });
    });
...
// Behavior:
// - If data is FRESH (< 30s old): Return cache immediately, trigger background scrape
// - If data is STALE (> 30s old): Wait for fresh scrape, then return
// - This ensures first visit/refresh gets fresh data, but navigation is fast
export async function getAllChannels(forceRefresh = false): Promise<{
  ...
  const MIN_SCRAPE_INTERVAL = 30; // 30 seconds - data is "fresh" if younger than this
  ...
  // If data is fresh and we have it, return immediately (navigation case)
  if (!forceRefresh && isDataFresh && dbMessages.length > 0) {
  ...
  // Data is stale or force refresh - we need fresh data
  // If a scrape is already in progress, wait for it
  if (isScrapingInProgress && scrapingPromise) {
    console.log(`[SUPABASE] Scrape in progress, waiting...`);
    try {
      ...
      console.error('[SUPABASE] Error waiting for scrape:', error);
    }
  }
  
  // If we have stale data but need fresh, start scrape and wait
  if (dbMessages.length > 0 && needsScrape) {
    console.log(`[SUPABASE] Data is stale (${cacheAge}s old) - scraping for fresh data...`);
  ...
  // No data at all - first time, must scrape
  if (dbMessages.length === 0) {
    console.log(`[SUPABASE] No cached data - scraping...`);
    isScrapingInProgress = true;
    try {
      const result = await fetchAndMergeData();
      return {
