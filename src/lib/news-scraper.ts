...
// RSS/Atom Feed parser for more reliable news fetching
async function fetchRSSFeed(feedUrl: string, sourceName: string, category: string): Promise<NewsArticle[]> {
  try {
    ...
    // Detect if this is Atom format (The Verge, etc.) or RSS format
    ...
    $(itemSelector).each((index, element) => {
      ...
      let link: string;
      if (isAtom) {
        link = $item.find('link[rel="alternate"]').attr('href') || $item.find('link').attr('href') || '';
      } else {
      ...
      if (title && link) {
        // Parse pubDate for actual publish time, fallback to current time
        let articleTimestamp: string;
        try {
          articleTimestamp = pubDate ? new Date(pubDate).toISOString() : new Date().toISOString();
        } catch {
          articleTimestamp = new Date().toISOString();
        }
...
// Main function to get news for a category (OPTIMIZED FOR SPEED)
// - Always return cached data immediately if available
// - Trigger background scrape if stale (don't wait)
// - Only wait for scrape if no data at all OR forceRefresh=true
export async function getNews(category: string, forceRefresh = false): Promise<NewsArticle[]> {
  // Check memory cache first (5 minute TTL) - but skip if forceRefresh
  ...
  const needsRefresh = forceRefresh || shouldScrape(lastScrape);

  // If forceRefresh=true, ALWAYS wait for fresh scrape
  if (forceRefresh) {
    console.log(`[NEWS] Force refresh requested for ${category} - scraping fresh data...`);
    const articles = await scrapeNews(category);
    memoryCache[category] = { articles, timestamp: Date.now() };
    return articles;
  }
  ...
  if (dbArticles.length > 0) {
    ...
    // If data is fresh, no need to scrape
    if (!needsRefresh) {
      console.log(`[NEWS] Using cached data for ${category} (${dbArticles.length} articles)`);
      return dbArticles;
    }

    // Data is stale - trigger background scrape but return cache immediately
    console.log(`[NEWS] Data stale for ${category} - returning cache, scraping in background...`);
    triggerBackgroundNewsScrape(category);
    return dbArticles;
  }

  // No data at all - must scrape (first visit)
  console.log(`[NEWS] No cached data for ${category} - scraping (first visit)...`);
  const articles = await scrapeNews(category);
...
async function triggerBackgroundNewsScrape(category: string): Promise<void> {
  // Don't start if already scraping this category
  if (activeScrapes.has(category)) return;

  activeScrapes.add(category);

  // Run in background (don't await)
  (async () => {
    try {
      console.log(`[NEWS] Background scrape started for ${category}...`);
      const articles = await scrapeNews(category);
      // Update memory cache with fresh data
      memoryCache[category] = { articles, timestamp: Date.now() };
      console.log(`[NEWS] Background scrape completed for ${category}!`);
    } catch (error) {
      console.error(`[NEWS] Background scrape error for ${category}:`, error);
    } finally {
      activeScrapes.delete(category);
    }
  })();
}

// Actual scraping logic
async function scrapeNews(category: string): Promise<NewsArticle[]> {
